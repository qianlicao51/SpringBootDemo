...
public class AOPConfig {
    ...
    public Object simpleAop(final ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            log.info("kind:{}", kind);//kind:method-execution
            ...
            log.info("被访问的方法名:{}", pjp.getSignature().getName());//方法名
            log.info("被访问的类名是:{}", pjp.getSignature().getDeclaringTypeName());//类名

            RequestMapping annotations = target.getClass().getDeclaredMethod(pjp.getSignature().getName()).getAnnotation(RequestMapping.class);
            if (annotations != null) {
